...
def register():
    ...
    if form.validate_on_submit():
        user = User(username=form.username.data, usertype=form.usertype.data, email=form.email.data, password=form.password.data)
        db.session.add(user)
        db.session.commit()
...
def post_jobs():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('posted_jobs'))
    return render_template('post_jobs.html', form=form, Random_Review=get_random_reviews())
